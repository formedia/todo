...
import {
  ...
  updateFilterStatus,
  updateFilterUserId,
} from '@/app/lib/features/todoSlice';
...
import {
  ServerNotificationType as SN,
  ClientNotificationType as CN,
} from '@/app/lib/ws_notification_type';
...
export default function Todo() {
  ...
  const { isConnected, sendMessage } = useWebSocket<ServerNotification>(
  ...
  const handleUpdate = useCallback(
    (todo: TodoData, update: Partial<TodoData>) => {
      sendMessage(
        JSON.stringify({ type: CN.UPDATE, todo: { ...todo, ...update } }),
      );
